 ...
 * 규칙은 옮기기 전(`app/export/page.tsx`)과 같다 — 익명은 `/login`, DID-only는 데모 모드로
 * 그 자리에서 계산을 보여 주고 내려받기만 막힌다.
 *
...
export default async function ExportLayout({ children }: { children: React.ReactNode }) {
  ...
  if (didSession) {
    // 지갑 미연결(DID-only)에도 계산은 보여 준다 — 데모 시나리오는 지갑 없이 성립한다.
    // 내려받기만 "지갑 연결 필요"로 막힌다(세금 화면이 하던 데모 모드가 여기로 이사했다).
