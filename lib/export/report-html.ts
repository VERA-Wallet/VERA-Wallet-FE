...
function cellText(value: string | number | undefined, currency: string): string {
...
function quantityText(value: string | number | undefined): string {
  if (value === undefined) return "—";
 ...
 */
function groupedAmount(value: Decimal): string {
...
function plainCell(value: string | number | undefined): string {
  if (typeof value !== "number") return `<td class="num">${escapeHtml(String(value ?? "—"))}</td>`;
  const decimal = decimalOf(value);
  const text = decimal === null ? String(value) : groupedAmount(decimal);
  return `<td class="num${value < 0 ? " neg" : ""}">${escapeHtml(text)}</td>`;
}

/** 금액 단위 표기. 원화는 "원", 나머지는 통화 코드 그대로. */
function unitLabel(currency: string): string {
...
const STATUS_BADGE: Record<TaxEstimate["status"], { label: string; tone: "ok" | "warn" }> = {
...
function generatedAtText(iso: string): string {
