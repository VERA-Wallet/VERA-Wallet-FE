...
import { eventRepository, summaryProvider, taxEngine, taxEvidenceProvider } from "@/lib/composition-root.client";
...
import { halfOpenPeriodLabel, isGroundedPeriod, periodFilePart } from "@/lib/period";
import { exportEventAllowance, usePlan } from "@/lib/plan/use-plan";
...
export function ReportView({
  countryCode,
  taxYear,
  assumeEffective = false,
}: {
  countryCode?: string;
  /** 링크가 실어 온 귀속연도. 전역 선택이 없을 때(새로고침·직접 진입)의 기본값이다. */
  taxYear?: number;
  /** 시행 전 룰셋을 시행됐다고 가정하고 볼지. 내보내기 화면의 토글 상태를 링크가 실어 온다. */
  assumeEffective?: boolean;
} = {}) {
  const [events, setEvents] = useState<NormalizedEvent[]>([]);
  const [summary, setSummary] = useState<SummaryDTO | null>(null);
  const [error, setError] = useState<string | null>(null);
  // estimate는 **어느 요청의 답인지(key)** 와 함께 둔다. 로딩·실패는 이 값에서 파생하므로 효과 안에서
  // 동기 setState를 부를 일이 없다(React Compiler가 막는다). 실패는 `estimate: null`로 남긴다.
  const [estimateResult, setEstimateResult] = useState<{ key: string; estimate: TaxEstimate | null } | null>(null);
  ...
  // 체인 기록은 어느 연도의 답인지 함께 들고 있는다(내보내기 화면과 같은 이유 — 연도를 바꿀 때 setState 연쇄를 피한다).
  ...
  // 귀속연도는 전역 단일 소스를 따른다. 링크의 연도는 아직 아무도 고르지 않았을 때의 기본값이다.
  const [selectedYear] = useTaxYear(taxYear ?? fallbackYear);
  ...
  useEffect(() => {
    ...
    void taxEngine
      ...
      .then((rulesets) => {
        if (!active) return;
        setEffectiveYear(rulesets.find((ruleset) => ruleset.code === country)?.effectiveTaxYear);
      })
      .catch(() => { /* 시행연도는 가정 문구에만 쓴다 — 못 받아도 보고서는 나온다. */ });
    return () => { active = false; };
  }, [country]);

  // 지금 화면이 기다리는 계산의 키. 답이 이 키를 달고 와야 "준비됨"이다 — 연도를 바꾼 뒤 늦게 온 옛 답을 막는다.
  const estimateKey = country === null ? null : `${country}:${selectedYear}:${assumeEffective ? "assume" : "actual"}`;

  useEffect(() => {
    // 기간이 없는 요약(빈 지갑)은 계산할 것이 없다 — 요청하지 않고 아래 파생값이 "계산 결과 없음"으로 간다.
    if (!summary || country === null || estimateKey === null || !isGroundedPeriod(summary.period)) return;
    let active = true;
    void taxEngine
      ?.estimate({ country, taxYear: selectedYear, source: "wallet", ...(assumeEffective ? { assumeEffective: true } : {}) })
      .then((next) => {
        if (!active) return;
        setEstimateResult({ key: estimateKey, estimate: next });
        setViewedAt(new Date().toISOString());
      })
      .catch(() => {
        // estimate가 없어도 표지와 한계만 있는 보고서는 낼 수 있다 — 종이와 같은 규칙.
        if (active) setEstimateResult({ key: estimateKey, estimate: null });
      });
    return () => { active = false; };
  }, [summary, country, selectedYear, assumeEffective, estimateKey]);

  const estimate = estimateResult !== null && estimateResult.key === estimateKey ? estimateResult.estimate : null;
  const estimateState: EstimateState = summary === null || country === null
    ? "idle"
    : !isGroundedPeriod(summary.period)
      ? "failed"
      : estimateResult === null || estimateResult.key !== estimateKey
        ? "loading"
        : estimate === null
          ? "failed"
          : "ready";
  ...
  useEffect(() => {
    ...
    void taxEvidenceProvider
      ...
      .catch(() => { /* 기록 조회 실패가 보고서를 막지는 않는다 — 체인 기록 칸만 비운다. */ });
    return () => { active = false; };
  ...
  // 잠금은 내보내기 화면의 다운로드와 같은 규칙이다 — 이 화면은 URL로도 열리므로 스스로 한 번 더 판정한다.
  const subscribed = plan !== null;
  const billableCount = summary?.computableEventCount ?? 0;
  const downloadLocked = !subscribed || (summary !== null && billableCount > exportEventAllowance(plan));
  const ready = summary !== null && error === null && estimateState !== "idle" && estimateState !== "loading";
  ...
  const print = () => {
    ...
    setPrintError(outcome === "unavailable" ? "인쇄 창을 열지 못했습니다. 팝업 차단을 해제하거나 내보내기에서 XLSX로 내려받아 주세요." : null);
  };
  ...
  const header = (
    <>
      <Link href="/export" className="inline-flex items-center gap-1 text-sm font-semibold text-zinc-500">
        <ArrowLeft aria-hidden className="size-4 shrink-0" strokeWidth={2.5} />
        리포트
      </Link>
      <div className="mt-4 flex items-center justify-between gap-3">
        <p className="text-sm font-semibold text-primary-500">내보내기 · 보고서</p>
  ...
  return (
    <main data-surface="report-view" className="mx-auto min-h-dvh w-full max-w-md px-5 py-8">
      ...
      {summary === null && !error && <p className="mt-5 text-sm text-zinc-500">보고서를 만드는 중…</p>}
      {estimateState === "loading" && <p className="mt-5 text-sm text-zinc-500">계산을 불러오는 중…</p>}
          ...
          {estimate ? (
            ...
            <section data-surface="report-headline" className="mt-5 rounded-card border border-zinc-200 bg-zinc-50 p-5">
              ...
              <p className="mt-1 text-sm leading-6 text-zinc-600">
                가격·분류를 확정한 거래가 없어 신고 금액을 내지 못했습니다. 내보내기의 거래 원장(CSV·XLSX)으로 확인해 주세요.
              </p>
            </section>
          )}
                  ...
                  <>
                    ...
                    <p className="mt-2 text-xs leading-5 text-zinc-500">
                      {exceptionRows.length > EXCEPTION_ROW_LIMIT
                        ...
                        : "관련 거래의 "}
                      전체 목록은 내보내기의 XLSX <strong>예외</strong> 시트와 CSV 거래 원장에 있습니다.
                    </p>
                  </>
                )}
              </Card>
          ...
          <Card className="mt-5">
            ...
            {anchor && (
              <section data-surface="report-anchor" className="mt-4 rounded-card border border-primary-200 bg-primary-50/50 p-4">
                ...
                <Link
                  href={`/export/evidence/${anchor.merkleRoot}`}
