...
import type { ReportInputs } from "@/lib/tax/use-report-inputs";
...
/**
 * 세부와 설정 — 답 다음이다. 위에 두면 사용자가 답까지 스크롤해야 한다.
 * 리포트 화면에서는 전부 접어 둔다: 첫 화면은 금액과 그 근거만으로 끝나야 한다.
 */
export function CalcDetails({ inputs }: { inputs: ReportInputs }) {
  const {
    result,
    selected,
    rulesetsFailed,
  ...
  return (
    <>
      ...
      {result && !hasNothingToCompute ? (
        <details className="mt-6 rounded-card border border-zinc-200 bg-white p-4 shadow-card" aria-label="계산 내역">
          <summary className="cursor-pointer font-bold text-zinc-900 marker:text-zinc-400">계산 내역</summary>
      ...
      {selected ? (
        <details className="mt-4 rounded-card border border-zinc-200 bg-white p-4 shadow-card" aria-label="확정 상태">
      ...
      {result ? (
        <details className="mt-4 rounded-card border border-zinc-200 bg-white p-4 shadow-card" aria-label="계산 설정">
          <summary className="cursor-pointer font-bold text-zinc-900 marker:text-zinc-400">계산 설정</summary>
      ...
      {result && requiresYearEndFmv ? (
        <details className="mt-4 rounded-card border border-zinc-200 bg-white p-4 shadow-card" aria-label="연말 시가 입력">
          <summary className="cursor-pointer font-bold text-zinc-900 marker:text-zinc-400">연말 시가 입력 (의제취득가액)</summary>
      ...
      <details className="mt-4 rounded-card border border-zinc-200 bg-white p-4 shadow-card" aria-label="계산 조건">
