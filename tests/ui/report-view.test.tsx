...
const ports = vi.hoisted(() => ({ list: vi.fn(), getSummary: vi.fn(), getProof: vi.fn(), estimate: vi.fn(), listRuleSets: vi.fn(), latest: vi.fn() }));
...
/** 보고서는 리포트 한 벌의 하위 화면이다. 프로덕션과 같이 프로바이더 하나 아래에서 estimate를 읽는다. */
function renderReport(options: { walletConnected?: boolean } = {}) {
  return renderReportPages({ pages: [], countryCode: "KR", currentYear: 2027, latestActivityYear: 2027, ...options, children: <ReportView /> });
}
...
describe("보고서 앱 화면", () => {
  it("종이와 같은 순서로 표지·신고 요약·자산별 명세·예외·한계를 앱 컬럼 안에 그린다", async () => {
    ...
    // 프로바이더가 서버가 내려준 거주국·귀속연도로 estimate를 요청한다. 보고서는 그 답을 그대로 읽는다.
  ...
  it("지갑 미연결(DID-only)에는 보고서로 만들 거래가 없다. 파일과 같은 이유를 말하고 문서를 만들지 않는다", async () => {
    renderReport({ walletConnected: false });

    expect(await screen.findByText(/지갑을 연결하면 내 거래로 신고 근거자료를 만들 수 있습니다/)).toBeInTheDocument();
    expect(screen.queryByText("1. 신고 요약")).toBeNull();
    expect(screen.queryByRole("button", { name: "PDF로 저장" })).toBeNull();
  });

  it("리포트 메인의 보고서 카드가 앱 화면(/export/report)으로 잇는다", async () => {
    renderReportPages({ pages: ["main"], countryCode: "KR", currentYear: 2027, latestActivityYear: 2027 });
    await screen.findByText("기타소득 계산");

    expect(await screen.findByRole("link", { name: "보고서 보기" })).toHaveAttribute("href", "/export/report");
