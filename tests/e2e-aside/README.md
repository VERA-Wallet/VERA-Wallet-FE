...
| 20 | 대시보드 `/dashboard` | 헤더·요약 카드·기간 선택·거래 목록 렌더, 분류 배지, 행 클릭 상세, 빈 상태(DID-only) CTA, 하단 탭 활성 표시, 가로 스크롤 없음 |
...
| 50 | 룰셋 비교 `/tax` | 연도 선택, 룰셋 카드 렌더, 판정 배지, 지갑 미연결 데모 문구, 면책 문구 고정 |
| 60 | 플랜 `/plan` | 탭 노출(곁길), 플랜 카드·과세연도, 다열 그리드 없음(448px 폭 규칙) |
| 70 | 내보내기 `/export` | 잠금 배너/허용 상태, 컬럼·요약, DID-only 빈 상태 CTA→/connect-wallet |
| 80 | 내비·404·공통 | 4탭 링크 이동·aria-current, 온보딩 화면에서 탭 숨김, 미존재 경로 404 뷰, 면책 푸터 sticky, 모든 화면 가로 오버플로 없음 |
