...
describe("금액 표기는 값을 숨기지 않는다", () => {
  ...
  it("원화 화면은 원 단위로 반올림한다", () => {
    // 표시만 반올림하며 계산 원문은 바꾸지 않는다.
    expect(formatFiat("13130.5", "KRW")).toBe("₩13,131");
  });
...
describe("금액 표기가 값을 바꾸지 않는다", () => {
  ...
  it("1원 미만의 원화는 0원으로 숨기지 않는다", () => {
    // KRW 관례는 0자리다. 그렇다고 0.005를 ₩0으로 지우면 값을 숨긴 것이다.
    expect(formatFiat("0.005", "KRW")).toBe("₩1 미만");
    expect(formatFiat("0.004", "KRW")).toBe("₩1 미만");
    expect(formatFiat("-0.14", "KRW")).toBe("-₩1 미만");
    expect(formatFiat("13130.5", "KRW")).toBe("₩13,131");
  });
