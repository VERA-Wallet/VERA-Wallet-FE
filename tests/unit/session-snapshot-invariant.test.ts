...
const beWithExpiry: SessionSnapshot = { source: "be", didVerified: true, countryCode: "KR", walletAddress: null, didExpiresAt: 1 };
// @ts-expect-error mock 스냅샷은 DID와 wallet 만료 시각을 모두 가져야 한다.
const mockWithoutExpiry: SessionSnapshot = { source: "mock", didVerified: true, countryCode: "KR", walletAddress: null, didExpiresAt: 1 };
...
describe("session snapshot invariants", () => {
  it("applies verification and completion rules to every snapshot variant including expiry boundaries", () => {
    ...
    const be: SessionSnapshot = { source: "be", didVerified: true, countryCode: "KR", walletAddress: "0x123" };
    const mock: SessionSnapshot = { source: "mock", didVerified: true, countryCode: "KR", walletAddress: "0x123", didExpiresAt: 11, walletExpiresAt: 12 };
