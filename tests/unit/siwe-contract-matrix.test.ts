...
describe("SIWE contract matrix", () => {
  ...
  it("requires all six onboarding conditions and blocks production test login", async () => {
    const complete = { didVerified: true, countryCode: "KR", walletAddress: account.address, chainId: 1, didExpiresAt: 2, walletExpiresAt: 2 };
    expect(isCompletedOnboarding(complete, 1)).toBe(true);
    for (const partial of [{ ...complete, didVerified: false }, { ...complete, countryCode: null }, { ...complete, walletAddress: null }, { ...complete, chainId: null }, { ...complete, didExpiresAt: null }, { ...complete, walletExpiresAt: null }, { ...complete, didExpiresAt: 1 }, { ...complete, walletExpiresAt: 1 }]) expect(isCompletedOnboarding(partial, 1)).toBe(false);
